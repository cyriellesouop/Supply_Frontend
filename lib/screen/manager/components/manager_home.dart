...
//pour le appBar
  ...
  UserModel deliver = UserModel(name: 'audrey', idDoc: 'audrey');
  var Dev = UserModel(name: 'audrey', idDoc: 'audrey');
  Map<String, dynamic> tab = {
    'Deliver': UserModel(name: 'audrey', idDoc: 'audrey'),
    'Distance': 0
  };
...
//
  ...
  getUserPosition() async {
    ...
    print(
        ' ${firs.locality},${firs.subLocality}, ${firs.administrativeArea}, ${firs.subAdministrativeArea},${firs.street}, ${firs.name},${firs.thoroughfare}, ${firs.subThoroughfare}');
    print(
...
//Maquer le manager courant
    ...
    await ServiceUser.getDelivers().forEach((element) async {
      ...
      for (var i in this.exampleModelDeliver) {
        n++;
        await ServicePosition.getPosition(i.idPosition).then((value) async {
            ...
            () {
              ...
              markers.add(Marker(
                  ...
                  infoWindow: InfoWindow(
                    //popup info
                    title:
                        'Livreur ${i.name} A ${getDistance(this.currentManagerPosition, coordonnees)} km de vous',
                          ...
                          Expanded(
                            ...
                            child: StreamBuilder<Object>(
                                ...
                                builder: (context, AsyncSnapshot snapshot) {
                                  ...
                                  //     var obj = snap.last.data();

                                  /*    var obj = snapshot.data((doc) {
                                    ...
                                    itemBuilder:
                                     ...
                                     /* PositionService()
                                      ...
                                      return InkWell(
                                          ...
                                          onTap: () async {
                                            print(
                                                "les distances sont ${distance.toString()}");
                                            await PositionService()
                                              ...
                                              setState(() {
                                               ...
                                               // dist = _getUserDistance(tableauJson,user);
                                                 dist = getDistance(
                                                    currentManagerPosition,
                                                    coordonnees); 
                                              });
                                            ...
                                            setState(() {
                                              isdeliver = false;
                                              deliver = users;
                                            //  dist = _getUserDistance(tableauJson,deliver);
...
// boite de dialogue pour l'edition de la commande
  ...
  getPosition(UserModel users, LatLng posiition) async {
    Map<String, dynamic> tableauJson = {};
    await ServicePosition.getPosition(users.idPosition).then((value) {
      LatLng coordonnees = LatLng(value.latitude, value.longitude);
      setState(() {
        tableauJson = {
          //json.decode9tableaujson[i]['Deliver']
          "Deliver": users.toMap(),
          "Distance": getDistance(posiition, coordonnees)
        };
      });
    });
...
//pour le appBar
  bool isSearching = false;
  var deliver = UserModel(name: 'audrey',idDoc: "audrey").toMap();
  var Dev = UserModel(name: 'audrey',idDoc: "audrey");
  Map<String, dynamic> tab = {
    'Deliver': UserModel(name: 'audrey',idDoc: "audrey"),
    'Distance': 0
                              ...
                              itemBuilder: (BuildContext context, int index) {
                                return InkWell(
                                  ...
                                  onTap: () {
                                    ...
                                    deliver != UserModel(name: 'audrey',idDoc: "audrey").toMap()
                                        ? isdeliver = true
                                        : isdeliver = false;
                                    print(
                                        'livreur est : $deliver et $isdeliver');
                                    if (deliver !=
                                        UserModel(name: 'audrey',idDoc: "audrey").toMap()) {
                                      isdeliver = true;
                                      /* setState(() {
